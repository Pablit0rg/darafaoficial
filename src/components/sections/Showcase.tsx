...
// Extraímos os dados para manter a renderização limpa e facilitar futura integração com CMS
const showcaseItems = [
  { id: 1, title: "Anel Bruto .01", desc: "Imagem Vertical (Peça Destaque)", spanClasses: "row-span-1 md:row-span-2" },
  { id: 2, title: "Colar Elos", desc: "Close-up Textura", spanClasses: "" },
  { id: 3, title: "Brinco Minimal", desc: "Detalhe Acabamento", spanClasses: "" },
  { id: 4, title: "Coleção 'Sombra'", desc: "Editorial / Modelo", spanClasses: "md:col-span-2 lg:col-span-1 lg:row-span-2" },
  { id: 5, title: "Pulseira Trama", desc: "Pulseira Prata", spanClasses: "" },
];
  ...
  return (
    <section className="py-20 px-4 md:px-12 bg-black">
      ...
      >
        ...
        <span className="text-xs text-gray-500 uppercase tracking-widest">Placeholders</span>
      </motion.div>
        ...
        {showcaseItems.map((item, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-6">
              <p className="text-sm font-sans tracking-widest uppercase text-white">{item.title}</p>
            </div>
          </motion.div>
        ))}
      </div>
